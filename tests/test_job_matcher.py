...
async def test_score_job_returns_high_score():
    ...
    with patch("core.job_matcher._client") as mock_client:
        mock_client.chat.completions.create = AsyncMock(return_value=_mock_openai_response(mock_content))
...
async def test_score_job_returns_none_on_bad_json():
    with patch("core.job_matcher._client") as mock_client:
        mock_client.chat.completions.create = AsyncMock(return_value=_mock_openai_response("bad json"))
        result = await score_job(PROFILE, JOB)
...
async def test_score_jobs_batch_filters_by_threshold():
    ...
    async def mock_create(**kwargs):
        nonlocal call_count
        resp = _mock_openai_response(scores[call_count])
        call_count += 1
        return resp
